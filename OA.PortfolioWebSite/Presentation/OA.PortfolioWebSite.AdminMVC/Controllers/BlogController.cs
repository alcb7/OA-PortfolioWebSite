...
namespace OA.PortfolioWebSite.AdminMVC.Controllers
{
    ...
    public class BlogController : Controller
    {
        ...
        private readonly string _apiBaseUrl = "https://dataapi.digigokali.com.tr/api/BlogPosts";
        ...
        {
            var projects = await _httpClient.GetFromJsonAsync<List<BlogViewModel>>(_apiBaseUrl);
        ...
        {
            var project = await _httpClient.GetFromJsonAsync<BlogViewModel>($"{_apiBaseUrl}/{id}");
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(int id, BlogViewModel project, IFormFile image)
        {
            if (id != project.Id)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(BlogViewModel project, IFormFile image)
        {
            if (ModelState.IsValid)
            {
                if (image != null)
                {
                    var response = await _httpClient.PostAsync("https://file.digigokali.com.tr/api/File/upload", new MultipartFormDataContent
                    {
                        { new StreamContent(image.OpenReadStream()), "file", image.FileName }
                    });
